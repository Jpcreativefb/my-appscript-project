...
assert(/^1\.2\.19-rc\.[2345678]$/.test(pkg.version), 'package version must identify rc2 or a certified successor');
assert(routes.include.includes('/api/app'), 'Cloudflare /api/app route must remain enabled');
assert(/v1219rc(2-performance-certification|3-final-performance|4-cache-persistence|5-admin-question-performance|admin-question-ux-performance)/.test(app), 'app asset marker must identify rc2 or its successor');
assert(/v1219rc(2-performance-certification|3-final-performance|4-cache-persistence|5-admin-question-performance|admin-question-ux-performance)/.test(sw), 'service worker cache must identify rc2 or its successor');
...
assert(/CacheService\.getScriptCache\(\)\.put\(key, username, (120|300)\)/.test(auth), 'session cache must remain short-lived and revocation-aware');
...
assert(/CacheService\.getScriptCache\(\)\.put\(scriptCacheKey, serialized, (300|900|1800)\)/.test(leagues), 'league cache TTL must remain bounded');
...
assert(/CacheService\.getScriptCache\(\)\.put\(cacheKey, serialized, (300|1800)\)/.test(appearance), 'appearance bundle should use bounded cache');

assert(/CacheService\.getScriptCache\(\)\.put\(coreCacheKey, serialized, (300|900|1800)\)/.test(reality), 'Reality TV core cache should remain bounded');
...
assert(/v1\.2\.19-rc(2|3|4)/.test(status), 'production status must identify rc2 or its successor');
