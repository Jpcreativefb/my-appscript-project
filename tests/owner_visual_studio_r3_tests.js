...
function server() {
  ...
  const frontend = { fetch:async()=>({json:async()=>identity}), console, sessionStorage: { removeItem() {}, setItem() {}, getItem() { throw new Error('Editable Draft must not come from client storage'); } } };
  frontend.window = frontend;
  frontend.api = async (action, payload) => {
    if(action==='studioEnvironment')return identity;
  ...
  const ready = R3.verifyDevelopmentEnvironment(frontend, identity, identity.apiUrl);
  return { ready, adapter: R3.serverAdapter(frontend), events, tables,
...
async function main() {
  const backend = server(); await backend.ready; let ui = controller(backend), studio = ui.controller;
