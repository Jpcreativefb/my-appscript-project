...
need(picks.includes('function refreshPicksAppearanceUi_()'), 'Shared Picks appearance redraw helper missing');
need((picks.match(/refreshPicksAppearanceUi_\(\);/g)||[]).length >= 2, 'Deferred and cached appearance paths must redraw the active Picks/Confidence surface');
need(picks.includes('if (shouldRenderCompactConfidenceSlate_())'), 'Shared redraw must preserve the compact Confidence branch');
need(picks.includes('list.innerHTML = renderCompactConfidenceSlate_();'), 'Confidence redraw must still target compact slate only');
