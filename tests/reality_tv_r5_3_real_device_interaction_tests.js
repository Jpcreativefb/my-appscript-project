...
// 8) Compare always keeps the signed-in player as the permanent first player column.
{
  const rows = [
    { username: 'joel', displayName: 'Joel', isCurrent: true },
    { username: 'amy', displayName: 'Amy' },
    { username: 'sam', displayName: 'Sam' }
  ];
  const standings = [{ username: 'joel', isCurrent: true }];
  const ctx = runFunctions(['realityTvCompareUserKey_', 'realityTvCompareCurrentUserKey_', 'realityTvCompareSelectedRows_'], {
    PICKS_PAGE_DATA: { session: { username: 'joel' } },
    PICKS_REALITY_COMPARE_USER_KEYS: ['joel','sam','amy','sam'],
    PICKS_REALITY_COMPARE_USERS_INITIALIZED: true,
    normalizeId: value => String(value || '').trim().toLowerCase()
  });
  const selected = ctx.realityTvCompareSelectedRows_(rows, standings);
  assert.strictEqual(JSON.stringify(Array.from(selected, row => row.username)), JSON.stringify(['joel','sam','amy']));
  assert.strictEqual(JSON.stringify(Array.from(ctx.PICKS_REALITY_COMPARE_USER_KEYS)), JSON.stringify(['sam','amy']), 'current user must never live in removable comparison-user state');

  const fresh = runFunctions(['realityTvCompareUserKey_', 'realityTvCompareCurrentUserKey_', 'realityTvCompareSelectedRows_'], {
    PICKS_PAGE_DATA: { session: { username: 'joel' } },
    PICKS_REALITY_COMPARE_USER_KEYS: [], PICKS_REALITY_COMPARE_USERS_INITIALIZED: false,
    normalizeId: value => String(value || '').trim().toLowerCase()
  });
  assert.strictEqual(JSON.stringify(Array.from(fresh.realityTvCompareSelectedRows_(rows, standings), row => row.username)), JSON.stringify(['joel']), 'first Compare open should auto-load only the signed-in reference player');
}

// 9) Add/remove keeps Compare open, tab-active, stable ordering, scroll position, and never removes current user.
{
  const states = [];
  const base = {
    PICKS_PAGE_DATA: {
      session: { username: 'joel' },
      episodeComparison: { rows: [{ username:'joel', isCurrent:true }, { username:'amy' }, { username:'sam' }] },
      realityTvView: { playerStats: { compactLeaderboard: [{ username:'joel', isCurrent:true }] } }
    },
    PICKS_REALITY_COMPARE_USER_KEYS: [], PICKS_REALITY_COMPARE_USERS_INITIALIZED: false,
    PICKS_REALITY_STANDINGS_COMPARE_TAB: 'standings', PICKS_REALITY_SECTION_OPEN_STATE: {},
    normalizeId: value => String(value || '').trim().toLowerCase(),
    realityTvCompareCurrentUserKey_: () => 'joel',
    realityTvCaptureComparisonUiState_: () => ({ open:true, scrollLeft:137 }),
    rerenderRealityTvEpisodeComparison_: state => states.push({ state: Object.assign({}, state) })
  };
  const ctx = runFunctions(['addRealityTvCompareUser_', 'removeRealityTvCompareUser_'], base);
  ctx.addRealityTvCompareUser_('amy');
  ctx.addRealityTvCompareUser_('sam');
  assert.strictEqual(JSON.stringify(Array.from(ctx.PICKS_REALITY_COMPARE_USER_KEYS)), JSON.stringify(['amy','sam']));
  assert.strictEqual(ctx.PICKS_REALITY_STANDINGS_COMPARE_TAB, 'compare');
  assert.strictEqual(ctx.PICKS_REALITY_SECTION_OPEN_STATE['standings-compare'], true);
  assert(states.every(item => item.state.open === true && item.state.scrollLeft === 137));
  ctx.removeRealityTvCompareUser_('amy');
  assert.strictEqual(JSON.stringify(Array.from(ctx.PICKS_REALITY_COMPARE_USER_KEYS)), JSON.stringify(['sam']), 'removing one user must preserve remaining comparison ordering');
  ctx.removeRealityTvCompareUser_('joel');
  assert.strictEqual(JSON.stringify(Array.from(ctx.PICKS_REALITY_COMPARE_USER_KEYS)), JSON.stringify(['sam']), 'current reference user must not be removable');
  ctx.addRealityTvCompareUser_('joel');
  assert.strictEqual(JSON.stringify(Array.from(ctx.PICKS_REALITY_COMPARE_USER_KEYS)), JSON.stringify(['sam']), 'current reference user must not be addable/reordered');
}

// 9b) Comparison rerender state captures/restores the open shell and horizontal position.
{
  const shell = { open: true };
  const wrap = { scrollLeft: 164 };
  const mount = { querySelector(selector) { return selector === '.reality-standings-compare-shell' ? shell : selector === '.reality-compare-matrix-wrap' ? wrap : null; } };
  let refreshed = 0;
  const ctx = runFunctions(['realityTvCaptureComparisonUiState_', 'realityTvRestoreComparisonUiState_'], {
    PICKS_REALITY_SECTION_OPEN_STATE: {}, PICKS_REALITY_COMPARE_SCROLL_LEFT: 0,
    document: { getElementById: id => id === 'realityTvEpisodeComparisonMount' ? mount : null },
    realityTvSetSectionOpen_: (host, open) => { host.open = open; },
    refreshRealityTvInteractionBridge_: () => { refreshed++; }
  });
  const captured = ctx.realityTvCaptureComparisonUiState_();
  assert.strictEqual(captured.open, true);
  assert.strictEqual(captured.scrollLeft, 164);
  shell.open = false; wrap.scrollLeft = 0;
  ctx.realityTvRestoreComparisonUiState_(captured);
  assert.strictEqual(shell.open, true);
  assert.strictEqual(wrap.scrollLeft, 164);
  assert.strictEqual(refreshed, 1);
}

// 10) Rendered Compare uses three-line player headers and frozen label/current-user columns at full section width.
{
  const render = functionSource(picks, 'renderRealityTvEpisodeComparison_');
  assert(render.includes('reality-compare-player-column-head ${isCurrent ? "is-current-reference" : ""}'));
  assert(render.includes('<strong>${escapeHtml(displayName)}</strong><span>${escapeHtml(realityTvFormatPoints_(total))} pts</span><small>${rank ? `Rank #'));
  assert(render.includes('${isCurrent ? "" : `<button type="button" class="reality-compare-remove-user"'));
  assert(render.includes('key !== currentKey && selectedKeys.indexOf(key) === -1'));
  assert(render.includes('data-reality-section-key="standings-compare" ${shellOpen ? "open" : ""}'));
  assert(picks.includes('realityTvCaptureComparisonUiState_'));
  assert(picks.includes('PICKS_REALITY_COMPARE_SCROLL_LEFT'));

  assert(css.includes('--reality-compare-label-width:92px'));
  assert(css.includes('--reality-compare-label-width:82px'));
  assert(css.includes('.reality-compare-grid-cell.is-current-reference'));
  assert(css.includes('left:var(--reality-compare-label-width)!important'));
  assert(css.includes('.reality-standings-compare-body{\n  padding:0!important'));
  assert(css.includes('--reality-compare-player-min:100px'));
