...
assert(/^1\.2\.19-rc\.[123]$/.test(pkg.version), 'package version must identify the v1.2.19 production candidate');
assert(/v1219rc[123]-(production-readiness|performance-certification|final-performance)/.test(app), 'app asset marker must include production candidate');
assert(/v1219rc[123]-(production-readiness|performance-certification|final-performance)/.test(sw), 'service worker marker must include production candidate');
assert(/prod=v1219rc[123]-(production-readiness|performance-certification|final-performance)/.test(html), 'app shell must force-refresh production candidate API/PWA assets');
