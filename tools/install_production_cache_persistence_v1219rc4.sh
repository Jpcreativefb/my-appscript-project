...
echo "=========================================="
echo " PATTC Predicts v1.2.19-rc4"
echo " Cache Persistence Certification"
echo "=========================================="
echo "Project: $REPO"
...
if [[ "$LOCAL" != ${EXPECTED_BASELINE}* ]]; then
  echo "STOP: RC4 requires baseline $EXPECTED_BASELINE but this repository is ${LOCAL:0:7}."
  exit 1
fi
...
done < "$PKG_ROOT/BASELINE_HASHES_V1_2_19_RC4.txt"
echo "BASELINE VERIFIED."

echo ""
echo "===== COPY CONTROLLED RC4 FILES ====="
FILES=(
  "BASELINE_HASHES_V1_2_19_RC4.txt"
  "CHANGED_FILES_V1_2_19_RC4.txt"
  "INSTALL_V1_2_19_RC4.txt"
  "PRODUCTION_STATUS.md"
  "backend/AuthEngine.js"
  "backend/engines/AppearanceEngine.js"
  ...
  "docs/PRODUCTION_CACHE_PERSISTENCE_V1_2_19_RC4.md"
  "frontend/app.html"
  "frontend/app.js"
  "frontend/js/app.js"
  ...
  "tools/install_production_cache_persistence_v1219rc4.sh"
...
chmod +x tools/install_production_cache_persistence_v1219rc4.sh tools/run_production_checks.sh
...
echo ""
echo "===== RC4 CACHE PERSISTENCE CONTRACT ====="
node tests/production_cache_persistence_v1219rc4_tests.js
...
echo ""
echo "===== COMMIT ====="
git commit -m "Stabilize PATTC Predicts cache persistence v1.2.19-rc4"
...
if ! git push origin "$EXPECTED_BRANCH"; then
  ...
  if [[ -n "$OLD_PROD_VERSION" ]]; then
    ...
    clasp deploy -i "$PROD_DEPLOYMENT" -V "$OLD_PROD_VERSION" -d "Rollback after RC4 GitHub push failure" || true
  fi
  exit 1
fi
