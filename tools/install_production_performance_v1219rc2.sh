...
echo "=========================================="
echo " PATTC Predicts v1.2.19-rc2"
...
if [[ "$LOCAL" != ${EXPECTED_BASELINE}* ]]; then
  echo "STOP: RC2 requires baseline $EXPECTED_BASELINE but this repository is ${LOCAL:0:7}."
  exit 1
fi
...
done < "$PKG_ROOT/BASELINE_HASHES_V1_2_19_RC2.txt"
echo "BASELINE VERIFIED."

echo ""
echo "===== COPY CONTROLLED RC2 FILES ====="
FILES=(
  "PRODUCTION_STATUS.md"
  "INSTALL_V1_2_19_RC2.txt"
  "CHANGED_FILES_V1_2_19_RC2.txt"
  "BASELINE_HASHES_V1_2_19_RC2.txt"
  "docs/PRODUCTION_PERFORMANCE_V1_2_19_RC2.md"
  "backend/AuthEngine.js"
  "backend/engines/AppearanceEngine.js"
  "backend/engines/DeviceSessionEngine.js"
  ...
  "tools/install_production_performance_v1219rc2.sh"
...
chmod +x tools/install_production_performance_v1219rc2.sh tools/run_production_checks.sh
...
echo ""
echo "===== RC2 PERFORMANCE CONTRACT ====="
node tests/production_performance_v1219rc2_tests.js
...
echo ""
echo "===== STAGE EXACT RC2 FILES ====="
git add -- "${FILES[@]}"
git status --short

# Refuse accidental unrelated changes.
...
echo ""
echo "===== COMMIT ====="
git commit -m "Certify PATTC Predicts performance v1.2.19-rc2"
...
if ! git push origin "$EXPECTED_BRANCH"; then
  ...
  if [[ -n "$OLD_PROD_VERSION" ]]; then
    ...
    clasp deploy -i "$PROD_DEPLOYMENT" -V "$OLD_PROD_VERSION" -d "Rollback after RC2 GitHub push failure" || true
  fi
  exit 1
fi
