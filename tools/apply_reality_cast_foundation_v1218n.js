...
const MARKER = 'v1218n-reality-cast-foundation';
...
const FRONTEND_CAST_FUNCTIONS = fs.readFileSync(path.join(ROOT, 'tools/reality_cast_frontend_foundation_v1218n.snippet'), 'utf8');
...
function bumpReleaseMarkers() {
  ...
  const sw = bumpVersionInFile('frontend/sw.js', 'AWARDS_CACHE');
  if (sw) {
...
function main() {
  ...
  // Release/cache markers are intentionally owned by the v1.2.18n automation patch.
  const runtimeList = Array.from(changed).sort();
  fs.writeFileSync(file('CHANGED_FILES_V1_2_18N_FOUNDATION_RUNTIME.txt'), runtimeList.join('\n') + '\n');
  console.log('Reality Cast foundation v1.2.18n applied.');
