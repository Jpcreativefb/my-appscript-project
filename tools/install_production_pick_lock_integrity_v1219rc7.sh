...
# Some Macs have clasp installed through a versioned Homebrew Node prefix and
# do not expose it on PATH in non-interactive installer shells.
...
echo "=========================================="
echo " PATTC Predicts v1.2.19-rc7"
echo " Pick Lock Integrity Certification"
echo "=========================================="
echo "Project: $REPO"
...
if [[ "$LOCAL" != ${EXPECTED_BASELINE}* ]]; then
  echo "STOP: RC7 requires baseline $EXPECTED_BASELINE but this repository is ${LOCAL:0:7}."
  exit 1
fi
...
done < "$PKG_ROOT/BASELINE_HASHES_V1_2_19_RC7.txt"
echo "BASELINE VERIFIED."

echo ""
echo "===== COPY CONTROLLED RC7 FILES ====="
FILES=(
  "BASELINE_HASHES_V1_2_19_RC7.txt"
  "CHANGED_FILES_V1_2_19_RC7.txt"
  "INSTALL_V1_2_19_RC7.txt"
  "PRODUCTION_STATUS.md"
  "backend/engines/AppDataEngine.js"
  "backend/engines/PicksEngine.js"
  "docs/PRODUCTION_PICK_LOCK_INTEGRITY_V1_2_19_RC7.md"
  "frontend/app.html"
  "frontend/app.js"
  "frontend/js/app.js"
  ...
  "tools/install_production_pick_lock_integrity_v1219rc7.sh"
...
chmod +x tools/install_production_pick_lock_integrity_v1219rc7.sh tools/run_production_checks.sh
...
echo ""
echo "===== RC7 PICK LOCK INTEGRITY CONTRACT ====="
node tests/production_pick_lock_integrity_v1219rc7_tests.js
...
echo ""
echo "===== COMMIT ====="
git commit -m "Enforce player pick lock integrity v1.2.19-rc7"
...
if ! git push origin "$EXPECTED_BRANCH"; then
  ...
  if [[ -n "$OLD_PROD_VERSION" ]]; then
    ...
    "$CLASP_BIN" deploy -i "$PROD_DEPLOYMENT" -V "$OLD_PROD_VERSION" -d "Rollback after RC7 GitHub push failure" || true
  fi
  exit 1
fi
