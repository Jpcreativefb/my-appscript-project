...
EXPECTED_BASELINE="9f93ec1"
PROD_DEPLOYMENT="AKfycbyDdfv-1xMQTL7LGhGp48_nmWqiNSvNcKLo5IHkAQTxsQCVIPaMP8ZlxMp0ZfT_bzvo"
RELEASE_DESC="v1.2.19-rc5 Admin Question Performance Certification"
...
echo "=========================================="
echo " PATTC Predicts v1.2.19-rc5"
echo " Admin Question Performance Certification"
echo "=========================================="
echo "Project: $REPO"
...
if [[ "$LOCAL" != ${EXPECTED_BASELINE}* ]]; then
  echo "STOP: RC5 requires baseline $EXPECTED_BASELINE but this repository is ${LOCAL:0:7}."
  exit 1
fi
...
done < "$PKG_ROOT/BASELINE_HASHES_V1_2_19_RC5.txt"
echo "BASELINE VERIFIED."

echo ""
echo "===== COPY CONTROLLED RC5 FILES ====="
FILES=(
  "BASELINE_HASHES_V1_2_19_RC5.txt"
  "CHANGED_FILES_V1_2_19_RC5.txt"
  "INSTALL_V1_2_19_RC5.txt"
  "PRODUCTION_STATUS.md"
  "backend/admin/AdminCategories.js"
  "backend/engines/NormalizedQuestionStorageEngine.js"
  "docs/PRODUCTION_ADMIN_QUESTION_PERFORMANCE_V1_2_19_RC5.md"
  "frontend/app.html"
  "frontend/app.js"
  "frontend/js/app.js"
  ...
  "frontend/sw.js"
  "package.json"
  "tests/production_admin_question_performance_v1219rc5_tests.js"
  ...
  "tools/install_production_admin_question_performance_v1219rc5.sh"
...
chmod +x tools/install_production_admin_question_performance_v1219rc5.sh tools/run_production_checks.sh
...
echo ""
echo "===== RC5 ADMIN QUESTION PERFORMANCE CONTRACT ====="
node tests/production_admin_question_performance_v1219rc5_tests.js
...
echo ""
echo "===== COMMIT ====="
git commit -m "Fix Admin question performance v1.2.19-rc5"
...
if ! git push origin "$EXPECTED_BRANCH"; then
  ...
  if [[ -n "$OLD_PROD_VERSION" ]]; then
    ...
    clasp deploy -i "$PROD_DEPLOYMENT" -V "$OLD_PROD_VERSION" -d "Rollback after RC5 GitHub push failure" || true
  fi
  exit 1
fi
...
echo ""
echo "=========================================="
echo " PATTC Predicts v1.2.19-rc5 INSTALLED"
echo " Production checks: PASS"
echo " Apps Script version: $NEW_VERSION"
echo " GitHub commit: ${COMMIT:0:7}"
echo ""
echo " NEXT: re-test Admin Categories / Questions / Nominees"
