...
RELEASE="v1.2.18p Team Fantasy Fast Preflight"

cd "$REPO"

echo ""
echo "=========================================="
echo " PATTC Predicts v1.2.18p"
echo " Team Fantasy Fast Preflight"
echo "=========================================="
echo "Project: $REPO"
...
if ! grep -q 'function teamFantasyPreflightIssues_' backend/engines/SportsTeamFantasyEngine.js; then
  echo "STOP: Team Fantasy preflight validator is missing."
  exit 1
fi
if ! grep -q 'TEAM_FANTASY_POSITIONS' backend/engines/SportsTeamFantasyEngine.js; then
...
echo ""
echo "===== APPLY v1.2.18p ====="
python3 "$PKG_ROOT/tools/apply_team_fantasy_fast_preflight_v1218p.py" "$REPO"
cp "$PKG_ROOT/tests/team_fantasy_fast_preflight_v1218p_tests.js" tests/team_fantasy_fast_preflight_v1218p_tests.js
cp "$PKG_ROOT/docs/TEAM_FANTASY_FAST_PREFLIGHT_V1_2_18P.md" docs/TEAM_FANTASY_FAST_PREFLIGHT_V1_2_18P.md
cp "$PKG_ROOT/CHANGED_FILES_V1_2_18P.txt" CHANGED_FILES_V1_2_18P.txt
cp "$PKG_ROOT/tools/apply_team_fantasy_fast_preflight_v1218p.py" tools/apply_team_fantasy_fast_preflight_v1218p.py
cp "$PKG_ROOT/tools/install_team_fantasy_fast_preflight_v1218p.sh" tools/install_team_fantasy_fast_preflight_v1218p.sh
chmod +x tools/apply_team_fantasy_fast_preflight_v1218p.py tools/install_team_fantasy_fast_preflight_v1218p.sh

echo ""
echo "===== JAVASCRIPT / PYTHON SYNTAX ====="
node --check backend/admin/AdminPreflight.js
node --check tests/team_fantasy_fast_preflight_v1218p_tests.js
python3 -m py_compile tools/apply_team_fantasy_fast_preflight_v1218p.py

echo ""
echo "===== TEAM FANTASY FAST PREFLIGHT TEST ====="
node tests/team_fantasy_fast_preflight_v1218p_tests.js
...
echo ""
echo "===== STAGE EXACT v1.2.18p FILES ====="
while IFS= read -r FILE; do
  [ -n "$FILE" ] && git add -A -- "$FILE"
done < CHANGED_FILES_V1_2_18P.txt

git status --short
if git diff --cached --quiet; then
  echo "STOP: Nothing staged for v1.2.18p."
  exit 1
fi

echo ""
echo "===== COMMIT ====="
git commit -m "Restore Team Fantasy fast preflight v1.2.18p"
...
echo ""
echo "=========================================="
echo " PATTC Predicts v1.2.18p COMPLETE"
echo ""
echo " Team Fantasy fast Run Check installed"
echo " Generic category loader bypassed for Team Fantasy"
echo " Normal game preflight preserved"
echo " Team Fantasy settings/picks/rules preserved"
