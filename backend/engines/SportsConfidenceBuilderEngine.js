...
function apiAdminCreateSportsConfidenceQuestionsBulk(payload) {
  ...
  const selected = sportsConfidenceParseSelectedGames_(payload);
  if (!selected.length) throw new Error("Select at least one sports game for Confidence.");
  if (selected.length > 64) throw new Error("Create at most 64 Confidence games at one time.");
  ...
  selected.forEach(function(item) {
    try {
      const result = createSportsConfidenceQuestionFromScore({
        awardsGameId: awardsGameId,
        sportsGameId: item && (item.sportsGameId || item.GameId || item.gameId),
        espnEventId: item && (item.espnEventId || item.ESPNEventId)
      });

      if (result && result.duplicate) duplicates.push(result);
      else created.push(result);
    } catch (err) {
      failed.push({
        sportsGameId: sportsConfidenceString_(item && (item.sportsGameId || item.GameId || item.gameId)),
        espnEventId: sportsConfidenceString_(item && (item.espnEventId || item.ESPNEventId)),
  ...
  return {
    ...
    message:
      "Confidence week build finished. Added: " + created.length +
      ". Already existed: " + duplicates.length +
