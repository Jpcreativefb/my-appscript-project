...
function renderRealityTvSpoilerShield_() {
  ...
  return `<section class="reality-spoiler-action ${resultsHidden ? "is-hidden" : "is-revealed"}" aria-live="polite" ${actionAttrs}>
    <div class="reality-spoiler-main-button">
...
function renderRealityTvPlayerSummary_() {
  ...
  if (view.playerStatsHiddenBySpoiler === true || (view.spoilerShield && view.spoilerShield.hasHiddenResults === true)) {
    return `<section class="reality-player-summary-card reality-your-season-card"><div class="reality-player-summary-heading"><div><h2>Your Season</h2></div></div><p>Your score and season stats appear after you reveal the settled episode.</p></section>`;
  }
  if (!view.playerStats || !view.playerStats.overall) return "";
  const overall = view.playerStats.overall || {};
  return `<section class="reality-player-summary-card reality-your-season-card">
    <div class="reality-player-summary-heading"><div><h2>Your Season</h2></div><div class="reality-player-score-total"><strong>${realityTvFormatPoints_(overall.totalPoints)}</strong><span>PTS</span></div></div>
  ...
  return `<details class="season-anchor-cast-bios">
    ...
    <div class="season-anchor-bio-browser ${densityClass}">${entities.map(function(item) {
      ...
      return `<article class="season-anchor-bio-card">
        ...
        <button type="button" class="reality-detail-action" onclick="showRealityTvContestantDetailModal_('${escapeJs(item.id)}')">Bio &amp; Details</button>
      </article>`;
    }).join("")}</div>
  </details>`;
}
...
function showSeasonAnchorStatsModal_() {
  ...
  wrapper.innerHTML = `<div id="seasonAnchorStatsModal" class="season-anchor-confirm-backdrop season-anchor-stats-backdrop" role="dialog" aria-modal="true" aria-labelledby="seasonAnchorStatsTitle">
    <div class="season-anchor-confirm-card season-anchor-stats-modal-card">
      <div class="season-anchor-stats-modal-heading"><div><span class="season-anchor-eyebrow">Sole Survivor</span><h2 id="seasonAnchorStatsTitle">More Stats</h2></div><button type="button" class="season-anchor-stats-close" aria-label="Close" onclick="closeSeasonAnchorStatsModal_()">×</button></div>
  ...
  modal.addEventListener("click", function(event) {
    if (event.target === modal) closeSeasonAnchorStatsModal_();
  });
}
...
function showRealityTvContestantDetailModal_(entityId) {
  ...
  wrapper.innerHTML = `<div id="realityTvContestantDetailModal" class="season-anchor-confirm-backdrop reality-contestant-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="realityContestantDetailTitle">
    <div class="season-anchor-confirm-card reality-contestant-modal-card" style="--reality-team-color:${escapeAttr(color)}">
      <div class="reality-contestant-modal-heading">
        ...
        <button type="button" class="season-anchor-stats-close" aria-label="Close contestant details" onclick="closeRealityTvContestantDetailModal_()">×</button>
      </div>
  ...
  modal.addEventListener("click", function(event) {
    if (event.target === modal) closeRealityTvContestantDetailModal_();
  });
}
      ...
      <div class="reality-sole-survivor-grid">
        ...
        <div class="reality-sole-survivor-right">
          ...
          <div class="reality-sole-survivor-actions">
            <button type="button" class="button secondary reality-sole-survivor-more-button" onclick="showSeasonAnchorStatsModal_()">More Stats</button>
            ${previewProfile ? `<button type="button" class="button secondary reality-sole-survivor-more-button" data-reality-contestant-id="${escapeAttr(previewProfile.id || currentId)}" onclick="showRealityTvContestantDetailModal_(this.dataset.realityContestantId)">Bio &amp; Details</button>` : ""}
          </div>
        </div>
      </div>
...
function realityTvCurrentCompareUserKey_(rows, standingsRows) {
  rows = Array.isArray(rows) ? rows : [];
  ...
  const currentKey = normalizeId(currentStanding && currentStanding.username) || sessionUser;
  if (!currentKey) return "";
  return rows.some(function(row) { return realityTvCompareUserKey_(row) === currentKey; }) ? currentKey : "";
}
...
function realityTvCompareSelectedRows_(rows, standingsRows) {
  ...
  const currentKey = realityTvCurrentCompareUserKey_(rows, standingsRows);
  const addedKeys = (PICKS_REALITY_COMPARE_USER_KEYS || []).filter(function(key) {
    return !!valid[key] && key !== currentKey;
  });
  PICKS_REALITY_COMPARE_USER_KEYS = (currentKey ? [currentKey] : []).concat(addedKeys);
...
function rerenderRealityTvCompareMount_() {
  const mount = document.getElementById("realityTvEpisodeComparisonMount");
  if (!mount) return;
  const previousShell = mount.querySelector && mount.querySelector(".reality-standings-compare-shell");
  const previousWrap = mount.querySelector && mount.querySelector(".reality-compare-matrix-wrap");
  const wasOpen = !!(previousShell && previousShell.open);
  const scrollLeft = Number(previousWrap && previousWrap.scrollLeft || 0);
  mount.innerHTML = renderRealityTvEpisodeComparison_();
  const nextShell = mount.querySelector && mount.querySelector(".reality-standings-compare-shell");
  const nextWrap = mount.querySelector && mount.querySelector(".reality-compare-matrix-wrap");
  if (nextShell && wasOpen) nextShell.open = true;
  if (nextWrap && scrollLeft > 0) nextWrap.scrollLeft = scrollLeft;
}
...
function addRealityTvCompareUser_(userKey) {
  ...
  const view = PICKS_PAGE_DATA.realityTvView || {};
  const compareRows = Array.isArray(comparison.rows) ? comparison.rows : [];
  const standingsRows = view.playerStats && Array.isArray(view.playerStats.compactLeaderboard) ? view.playerStats.compactLeaderboard : [];
  const currentKey = realityTvCurrentCompareUserKey_(compareRows, standingsRows);
  ...
  rerenderRealityTvCompareMount_();
}
...
function renderRealityTvEpisodeComparison_() {
  ...
  const currentUserKey = realityTvCurrentCompareUserKey_(compareRows, rows);
  ...
  const availableToAdd = compareRows.filter(function(row) {
    const key = realityTvCompareUserKey_(row);
    return key && key !== currentUserKey && selectedKeys.indexOf(key) === -1;
  });
  ...
  const compareGrid = available && selectedRows.length ? (function() {
    const playerHeads = selectedRows.map(function(row) {
      ...
      const isCurrent = !!currentUserKey && key === currentUserKey;
      const removeButton = isCurrent ? "" : `<button type="button" class="reality-compare-remove-user" aria-label="Remove ${escapeAttr(displayName)} from comparison" onclick="removeRealityTvCompareUser_('${escapeJs(key)}')">×</button>`;
      return `<div class="reality-compare-grid-cell reality-compare-player-column-head${isCurrent ? " reality-compare-current-column" : ""}"><div class="reality-compare-user-head"><div class="reality-compare-user-summary"><strong>${escapeHtml(displayName)}</strong><span>${escapeHtml(realityTvFormatPoints_(total))} pts</span><span>${rank ? `Rank #${rank}` : "Rank —"}</span></div>${removeButton}</div></div>`;
    }).join("");
    const rowHtml = function(label, getter) {
      return `<div class="reality-compare-grid-cell reality-compare-question-label">${escapeHtml(label)}</div>${selectedRows.map(function(row) {
        ...
        return `<div class="reality-compare-grid-cell reality-compare-pick-cell${currentUserKey && key === currentUserKey ? " reality-compare-current-column" : ""}">${imageHtml}<strong>${escapeHtml(value)}</strong></div>`;
      }).join("")}`;
    };
...
function renderRealityTvRulesHelp_() {
  ...
  return `<details class="reality-help-bottom reality-help-shell" id="realityTvRulesHelp">
...
function renderRealityTvEpisodeSections_(categories) {
  ...
  const previousHtml = previous.length ? `<details class="reality-previous-episodes"><summary><span><strong>Previous Episodes</strong><small>${previous.length} revealed episode${previous.length === 1 ? "" : "s"}</small></span></summary><div class="reality-previous-episodes-body">${previous.map(function(episode) {
...
(function realityEnhancedCleanR3_(global) {
  ...
  function enhance_() {
    ...
    // Clean R3 owns the mutations below. Suspend its observer so those writes
    // cannot recursively schedule another enhance pass against the DOM it just built.
    suspendObserver_();
    try {
      const mode = layout_();
  ...
  function startObserver_() {
    ...
    if (!observer) observer = new MutationObserver(queue_);
    resumeObserver_();
  }
...
async function confidenceRc24aLoadCompletion_(){
  ...
  try{
    ...
    var res=await api('getSportsConfidenceCompletion',{username:session.username||'',gameId:PICKS_PAGE_DATA.gameId,leagueId:confidenceRc24aLeagueId_(),week:pattcConfidenceWeeklyEnabledR1_()?pattcConfidenceCurrentWeekR1_():0});
...
function confidenceRc24aSwitchWeek_(gameId,week){ if(pattcConfidenceWeeklyEnabledR1_()){var selected=Number(week)||0;if(selected&&typeof pattcConfidenceChooseWeekR1_==='function'){CONFIDENCE_RC24A_COMPLETION_STATE=null;pattcConfidenceChooseWeekR1_(selected);confidenceRc24aLoadCompletion_();}return;} if(!gameId)return; try{localStorage.setItem('gameId',gameId);}catch(e){} if(window.APP_STATE) APP_STATE.gameId=gameId; if(typeof navigate==='function') navigate('picks',{skipUnsavedCheck:true}); }
