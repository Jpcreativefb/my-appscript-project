...
(function(root){'use strict';
  ...
  root.renderSeasonHubPage=async function(){
    ...
    const breakdown=contribs.length?contribs.map(c=>`<div class="nfl-cup-contribution"><div><strong>${esc(c.gameName||c.gameId)}</strong><small>${c.placementRank?'#'+c.placementRank+' · ':''}${Number(c.seasonCupFieldSize)||0} entrants${c.seasonCupQualified===false?' · field below minimum':''}</small></div><b>${fmt(c.contribution)} Cup</b></div>`).join(''):'<div class="nfl-cup-empty">No qualifying Cup contributions yet.</div>';
    return `<div class="page season-hub-page nfl-cup-r2">
      ...
      <section class="nfl-cup-r2-summary"><div><span>YOUR PLACE</span><b>${me?'#'+(index+1):'—'}</b></div><div><span>COUNTED GAMES</span><b>${me?Number(me.miniGamesCounted)||0:0}</b></div><div><span>CUP FIELD</span><b>${rows.length}</b></div></section>
